...
| T-11 | **opencode 状态隔离、配置共享**：`XDG_STATE_HOME`→userData；不设 `OPENCODE_CONFIG_DIR` | 全隔离（pinpin 做法） | 徽章依赖 session 持久性→状态必须归 Sepia；D-14/D-36 要求凭据「随 opencode 现有配置」→全局配置必须共享 |
