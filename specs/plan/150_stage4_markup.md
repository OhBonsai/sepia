...
dead_checks: 3           # 反向验证首轮空转数（003 §3.2 口径，记首轮不记收尾）—— #4 原子性、#7 流式单调、#9 全链打点，均已修，明细见 §1.5
...
| a4 | 真模型一轮全链 | 真 key，打点数字记录 | **未跑**——不进 CI（001 §6），留给人跑；桩模型下六点齐、顺序对、在预算内 |
...
- [ ] **落笔手感**：⌘Z 一次撤干净，无半截态
- [ ] **流式揭示观感**：snap 节奏舒不舒服、结构块有没有闪一下 raw——机器断言只能证
