...
stage: 6
title: 库与文件
status: done          # a+b 两期均已关闭。a 期 2026-08-06 合并；b 期 2026-08-06 人工轮八项全通过后合并
dod_a: 外部改文件（无脏）→ 重载且尽量保光标；（有脏）→ 先落盘 + 横条提示；自写不自扰；删除进回收站；`open -a Sepia x.md` 游离打开可写；watcher 失效时 focus 对账仍工作
dod_b: 整个 art/ 作 book 启动仍 <1s；@ 按下到列表 <100ms；多 Tab 重启恢复 + 气质基线绿
checks_added: 47                # a 期 20（集成后 #2 删、#21 增，净数不变）+ b 期 27（§2.4 计划 13 + 走查/人裁期间新增或加严 14）
checks_reverse_verified: 43     # a 期 20/20；b 期 23/27——差的 4 条是 smoke 层未逐条破坏的（#6/#7/#7b/#12），记债见文末
dead_checks: 8         # a 期 3（§1.5）+ b 期 5：`@` 前缀优先与标题前缀两条**期望顺序恰好等于字母序兜底**、#10 旧版在事件层之下、#10d 第一版光标位置短路、#10g 第一版被 /var 与 /private/var 的差异弄成恒真。五条均已改到会红。另有两处不计入：`tabPath` 是**压根没有单测**（缺失≠空转，已补），`closeTab` 的越界守卫**不可观测**（去掉行为完全相同，是死代码不是死检查）
...
**2026-08-06 八项全部通过（人裁）。走查口径：dev 实例**（`bun dev`）——
这一点要写明，因为本期恰好有一条缺陷**只在 dev 态成立**（图片预览走 `file://`），
若人工轮走的是打包态，它会一路绿到发版。**此后人工轮默认在 dev 态走**，
需要打包态才能验的（.dmg / 双击关联）单独注明。

- [x] 文件树手感与收起 —— 2026-08-06
- [x] 多 Tab 切换与重启恢复 —— 2026-08-06
- [x] 主页两条路 + 最近列表 —— 2026-08-06（走查中撞出"点最近打不开"，当场修，见第二处）
- [x] `@` 即时感 —— 2026-08-06（走查中撞出浮层不跟光标，当场修，见第三处）
- [x] 拖图落盘 —— 2026-08-06（走查中撞出拖拽/粘贴双双无效 + 挤进别人行里，当场修，见第四、五处）
- [x] **⌘⌫ 删除 → Finder 废纸篓找到** —— 2026-08-06，**6a 债 A 就此还清**
- [x] 气质修正后的"纸感"直观判断 —— 2026-08-06
- [x] 真实 art/ 打开体感 —— 2026-08-06（走查中撞出空 book 只剩两个哑目录行，当场修，见第一处）

**八项里五项在走查中撞出了缺陷**（外加一项人裁新增 5b）。这个比例本身是条信号：
机器侧 21 条全绿的同时，人一走就出五处——记进 §1.9。
...
### 机器侧（人工轮结束时的终值，2026-08-06）

`bun run check` **PASS**（豁免 4 / dispute 0）｜ smoke **59 条全绿**
｜ core 单测新增 45（session 13 / library 25 / assets 7）。

人工轮期间新增 11 条 smoke（五处当场修 + 人裁 5b），所以"本期新增 8 条"是走查前的数。

### 与计划的三处偏离——**人裁 2026-08-06：三条全部接受**

| # | 人裁 |
|---|---|
| 1 | **接受 +6**。两项都是实现时才显形的结构性需要，不是顺手加的 |
| 2 | **接受降级**（⌘点击 = 新 tab）。按 §2.1 ④ 写死的条款走，**双屏随债池冻结至 MVP 关闭后** |
| 3 | **接受代价落点**。v2 的代价落在测试装置而不是用户身上，正是"未发布无兼容负担"想要的位置 |
...
## Stage 6 关闭（2026-08-06）

**a 期 + b 期一并关闭。** 人工轮八项全部通过（人裁），三处偏离全部接受。

### DoD_a 逐条判定

| 条款 | 判定 | 据什么 |
|---|---|---|
| 外部改文件（无脏）→ 重载且尽量保光标 | **达成** | 人工①（vim / VS Code 各改一次，重载 + 光标体感 ✓）+ smoke `external-change` |
| （有脏）→ 先落盘 + 横条提示 | **达成** | 人工①（字零丢失、横条如期出现）+ smoke 按 `data-sepia-conflict` 断言三态 |
| 自写不自扰 | **达成** | smoke「保存一次不自我重载」跑在真接缝下（桩已删）+ 实测「一次写 = 恰好 1 个事件」20/20 |
| 删除进回收站 | **达成，债 A 就此还清** | 人工轮第 6 项 2026-08-06 通过——**这是 6a 记债时明写的还款条件**（"b 期一绑键，第一个人工项就是它"）。机器侧 `files.test.ts` 断言调的是 trash 而不是 unlink |
| `open -a Sepia x.md` 游离打开可写 | **部分达成** | argv/第二实例那半有 smoke；**双击 .md 那半仍未验**（要打包，LaunchServices 不认未打包应用）——**债 B 未清，随 `.dmg` 专项**。不含糊记成"达成" |
| watcher 失效时 focus 对账仍工作 | **达成** | smoke 覆盖 focus 对账路径；`usePolling` 逃生舱有 smoke #6b 那一族 |

### DoD_b 逐条判定

| 条款 | 判定 | 据什么 |
|---|---|---|
| D1 整个 art/ 作 book：启动可写不被树挡（<1s） | **达成** | 机器 #13（t0–t5 攒齐、树在其后异步到）+ 人工轮第 8 项体感通过。冷启动实测 559ms |
| D2 `@` 按下到列表 <100ms | **达成（体感口径）** | 人工轮第 4 项通过。**机器证不了这个数**——smoke 只能证"列表出现且内容对"，<100ms 是人真敲一次的判断 |
| D3 多 Tab 重启恢复；气质基线绿 | **达成** | smoke #6（tabs/active/光标往返）+ #12（无行号槽、正文非等宽、版心 ≤760）+ 人工轮 1/2/7 |

### 一条留给 Stage 7 的账

**DoD_a 第五条只完成了一半**，且另一半的还款条件（打包）不在 Stage 6 手上。
它与债 B 是同一件事，**不因为 Stage 6 关闭而消失**——Stage 7 的 `.dmg` 专项里
第一个人工项就是它，与 6a 当时对债 A 的处置方式相同。

### 收尾补做的反向验证（2026-08-06）——**四条空转、一条缺失**

关闭前回头查 b 期的 RV 记录，发现**只有真人轮那五处修复有破坏记录**，
实施期的 13 条计划检查一条都没写下破坏结果。于是逐条补做（单测层很便宜，
纯函数不用起 Electron），当场抓到：

| 抓到什么 | 形态 |
|---|---|
| `@` **前缀优先**空转 | 期望顺序 `['arch.md','archive.md']` **恰好也是字母序兜底的顺序**——把前缀分支整个删掉，两者一起掉到"子串 70 分"再按 `localeCompare` 排，答案一模一样 |
| `@` **标题前缀**空转 | 同一个形态换到标题那一侧 |
| **`tabPath` 一条单测都没有** | 而真人轮"点最近打不开"正是它。删掉绝对路径直通那一行，21 条 session 单测全绿——真正拦住它的只有 smoke #8b |
| `closeTab` 越界守卫**不可观测** | 去掉之后行为完全相同（`filter` 对越界下标本就无事发生）。这是**死代码不是死检查**，不计入 `dead_checks`，也不为它编一条测试 |

修法都是同一个：**让两种排法给出相反的答案**。加了 `a-plan.md`（查 `plan` 时是子串
命中，而字母序排在 `plan.md` 前）与 `a-old.md`（标题那一侧的同款），再补一整块
`tabPath` / `tabRelative` 单测。补完再破坏，五处全红。

> **这条比五处缺陷更值得记**：前两条的空转形态是「**期望值恰好等于兜底行为的输出**」。
> 它躲得过肉眼——测试写得很像在断言排序，实际断言的是字母表。
> 判据：**写排序断言时，必须让被测规则与兜底规则给出不同答案**，
> 否则那条断言测的是兜底。建议升进 002。

### 人工轮的一条信号

**八项里五项在走查中撞出了缺陷**，而机器侧在同一时刻是全绿的。
五处分别是：空 book 只剩哑目录行、点最近打不开、`@` 浮层不跟光标、
拖拽/粘贴双双静默无效、图挤进别人行里；外加人裁新增的 dev 态预览。

它们的共同形状不是"功能没做"，而是**"功能对、但没按计划写的那个形态做"**——
计划里写着"行内浮层"、"说人话"、"共用一份换算口"，我把这些形容词当成了修饰，
而它们是规格。这条已进 §1.9，并建议升进 002。
