...
**〇 气质修正（开门第一批）**
- [ ] 去行号：baseExtensions 移除 `lineNumbers()`；同步修 editor 守卫测试与受影响 smoke
- [ ] 正文字体：`.cm-content` 改正文栈 `-apple-system, "PingFang SC", "Hiragino Sans GB", "Segoe UI", sans-serif`，16px / 1.85；**代码块与行内代码保持等宽**（syn-code 装饰单独 font-family）
- [ ] 版心：正文栏居中限宽 **760px**（浮层/diff 同宽约定自动继承）
- [ ] 杂件统一：错误横幅 / 冲突横条 / agent 线 / 警示点收进一个 CSS 组件族（同高度字号色板）
- [ ] smoke·气质基线（computed/像素级）：无 `.cm-gutters`、正文非 monospace、版心 ≤760。破坏：加回 lineNumbers / 字体改回 → 必红

**① session v2 + 多 Tab**
- [ ] schema：`{ version:2, book:string|null, tabs:[{page,cursor,scrollTop}], active:number }`；book 内 page 存 book 相对路径，游离存绝对
- [ ] tab 条：顶部细条（文件名+关闭点，无图标）；⌘O/树/@/argv 打开即新 tab（已开则聚焦）；⌘W 关、⌘⇧[ ⌘⇧] 切；每 tab 独立 cursor/scroll，切换重挂 EditorHost；脏标记逐 tab
- [ ] 重启恢复 tabs+active；全关显示主页

**② 文件树（6a 回流 1：有界 + 降级）**
- [ ] 侧边栏可全收起（⌘B）；树数据一次性**异步**扫描（不进启动同步路径，纪律 12；不递归 watch）
- [ ] 超 `treeEntryLimit` → 降级：只列当前层目录 + 顶部提示
- [ ] 刷新 = focus 对账 + **只 watch 展开中的目录**（各 depth:0，复用 6a watcher）
- [ ] 单击开 tab；右键菜单接 6a 四命令；**`files.trash` 绑 ⌘⌫**（还 6a 债 A 入口半）

**③ 主页完整版**
- [ ] 无 tab 时显示；主页也是纸（无插画无向导，版心同上）
- [ ] 无 book：「选择文件夹作为 book」（openDirectory）/「打开一个 .md」（游离）
- [ ] 有 book：最近 page 列表直达；recents 落 `books/<id>/recents.json`，打开置顶，`library.recentsLimit = 20`

**④ `@` 引用**
- [ ] 输入 `@` → 行内浮层：模糊匹配文件名+标题，数据用树扫描同一份内存清单；标题索引后台异步补建（同上限），未建好时纯文件名匹配也必须即时
- [ ] 选中插入 `[标题](book相对路径.md)`；点击在新 tab 打开
- [ ] 双屏：⌘点击 → 右侧只读分屏，Esc 关。**复杂度超预期即降级为新 tab 打开并记回流请人裁**——不许为它引依赖或重构 EditorHost

**⑤ 拖拽与图片（架构 §4.9 落点表）**
- [ ] 编辑区收图片/GIF：落盘 `book/img/<yyMMddHHmm>-<原名>` + 插入 `![](img/…)`；⌘V 粘图同路径
- [ ] 编辑区拖其他一切无效；窗口空白/图标/argv 收 .md → 开 tab（游离通道已有）

**⑥ 更新链接命令（T-31）**
- [ ] 重命名/移动后横条提示「N 处引用未更新 · 更新」——**用户主动点击**，不自动
- [ ] 执行前横条展开列将改动文件清单；确认 → 改写 + 同一 commit
